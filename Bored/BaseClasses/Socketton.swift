...
////  CoWorkerUser
////
////  Copyright © 2023 Abhishek Jaiswal. All rights reserved.
////
...
class Socketton: NSObject {
    ...
    var chatId: String = ""
    var userId: String? = UserDefaultsCustom.getUserData()?.id
    ...
    func setupSocket() {
        ...
        let userId = UserDefaultsCustom.getUserData()?.id
...
//        self.isSocketConnectedCallback = callback
    ...
    func sendMessage(_ dict: JSON, roomId: String) {
        self.checkConnection { conected in
            ...
            self.socket?.emit("newMessage", roomId, dict as! SocketData)
        }
    }
    ...
    func sendMessage(_ dict: JSON, roomIds: [String], completion: @escaping(()->())) {
        self.checkConnection { conected in
            ...
            roomIds.forEach { roomId in
                self.socket?.emit("newMessage", roomId, dict as! SocketData, completion: {
                    count = count + 1
                })
            }
        }
