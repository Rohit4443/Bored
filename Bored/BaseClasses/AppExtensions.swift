...
//        let date = Date(timeIntervalSince1970: timeStamp as? Double ?? 0.0)
    ...
    func convertStringToDate(myString: String, time: String, format: String) -> Date? {
        ...
        print("dateStrdateStrdateStr", myString,   format,     yourDate)
        return yourDate
    }
    
    func convertStringToDate(toDate: String, format: String) -> Date {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = format //"yyyy-MM-dd EEE"      /////////   "MM/dd/yyyy"
    ...
    func changeDateFormat(date: Date, format: String) -> Date? {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = format// format "DD-MM-YYYY"  "YYYY-MM-dd"
...
//        dateFormatter.dateFormat = format
    ...
    func getDateMonthYearFormat(date: Date, format: String) -> String? {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = format
        let resultString = dateFormatter.string(from: date)
//        dateFormatter.dateFormat = format
...
extension NSObject {
    ...
    func postDeletedPopup(vc: UIViewController?) {
        let alert = UIAlertController(title: "Elisium", message: "This post is removed", preferredStyle: .alert)
    ...
    func userDeletedPopup(vc: UIViewController?) {
        let alert = UIAlertController(title: "Elisium", message: "This user is no longer available.", preferredStyle: .alert)
        let alertAction = UIAlertAction(title: "Okay", style: .default)
        alert.addAction(alertAction)
        vc?.present(alert, true)
    }
