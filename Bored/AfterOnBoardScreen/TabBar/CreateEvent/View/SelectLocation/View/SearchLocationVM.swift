...
protocol SearchLocationVMObserver: NSObjectProtocol{
    func searchListing()
    func recentSearch()
...
class SearchLocationVM: NSObject{
    
    var observer: SearchLocationVMObserver?
    var searchListing: [searchLocationData] = []
    var recentSearchListing: [RecentSearchLocationData] = []
    var perPage : Int = 1000
    var pageNo  : Int = 1
    
    init(observer: SearchLocationVMObserver? = nil) {
        ...
        AFWrapperClass.sharedInstance.requestPOSTSURL(Constant.searchListing, params: SearchParams(search: search, isRecentSearch: isRecentSearch), success: {
            (response) in
                ...
                if let parsedData = try? JSONSerialization.data(withJSONObject: response, options: .prettyPrinted) {
                    do {
                        let userModel = try JSONDecoder().decode(SearchLocationModel.self, from: parsedData)
